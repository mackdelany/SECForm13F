...
class investmentFirm():
    ...
    def getAccNumbersFromCIK(self):
        url = "https://www.sec.gov/cgi-bin/browse-edgar?CIK=" + self.cik + "&action=getcompany&type=13F"
    ...
    def get13FfilingURLs(self, accNumbers):
        ...
        for accNo in accNumbers:
            url = "https://www.sec.gov/Archives/edgar/data/" + self.centralIndexKey + "/" + accNo + "/xslForm13F_X01/infotable.xml"
            URLs.append(url)

        return URLs

    def getDataframeOfFiling(self, url):
    ...
    def getAll13Ffilings(self):
        ...
        accNumnbers = getAccNumbersFromCIK()
        filingURLs = get13FfilingURLs(accNumnbers)
